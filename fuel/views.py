...
from .models import Combustivel, CadCarros, TipoCombustivel, Abastecimento, Consumo_carros
...
def abastecimentos(request):
    ...
    if request.method == 'GET':
        ...
        # consumo_cid_etanol = Consumo_carros.objects.filter(consumo_cidade_etanol=id)
        # consumo_cid_gasolina = Consumo_carros.objects.filter(modelos=id)
        # consumo_est_etanol = Consumo_carros.objects.filter(modelos=id)
        # consumo_est_gasolina = Consumo_carros.objects.filter(modelos=id)
        km_rodados = ""
        media = ""
        custo_km = ""
        custo_abastecimento = ""
        ...
        if data_abast_filtrar:
            ...
            for custo_abastecimento in data_abastecimento:  # Filtra resultados para 'custo_abastecimento'
                custo_t = custo_total.filter(user=request.user, data_abastecimento=data_abast_filtrar_dt).first().custo_total
                custo_abastecimento = (f"Custo Total: R${custo_t}")
            # for comparacao in media:
            #     etanol_cidade = consumo_cid_etanol.filter(consumo_cidade_etanol=request.user).first().consumo_cidade_etanol
            #     comp = media >= etanol_cidade
            #     comparacao = (f"Média correta: {comp}")
        else:
            ...
            # comparacao = "Média correta: Nenhum filtro aplicado"
...
def cad_carros(request):
    if not request.user.is_authenticated:
        ...
        modelos = Consumo_carros.objects.all()
        return render(request, 'cad_carros.html', {'tipocombustivel': tipo, 'modelocarro': modelos})
    elif request.method == 'POST':
        veiculo = request.POST.get('veiculo')
        ...
        modelos_id = request.POST.get('modelos')

    try:
        modelos = Consumo_carros.objects.get(id=modelos_id)
    except Consumo_carros.DoesNotExist:
        return render(request, 'cad_carros.html', {'error': 'Modelo de carro inválido'})
